...
namespace HoltsmarkPadeCoefGeneration {
    internal class Quantile {
        static void Main_() {
            ...
            using (StreamWriter sw = new("../../../../results_disused/pade_quantile_precision150_scaled.csv")) {
                bool approximate(MultiPrecision<Pow2.N64> pmin, MultiPrecision<Pow2.N64> pmax) {
                    ...
                    Vector<Pow2.N64> xs_forward = expecteds_range.Select(item => item.p - pmin).ToArray();
                    Vector<Pow2.N64> xs_backward = expecteds_range.Select(item => pmax - item.p).ToArray();
                    ...
                    for (int coefs = 5; coefs <= 100; coefs++) {
                        foreach ((int m, int n) in CurveFittingUtils.EnumeratePadeDegree(coefs, 2)) {
                            ...
                            foreach ((bool forward, Vector<Pow2.N64> xs) in new[] { (false, xs_backward), (true, xs_forward) }) {
                                if (pmin == 1 && !forward) {
                                    continue;
                                }
