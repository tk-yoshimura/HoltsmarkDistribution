...
namespace HoltsmarkPadeCoefGeneration {
    internal class PDFLimit {
        static void Main() {
            ...
            using (StreamWriter sw = new("../../../../results_disused/pade_pdflimit_precision151_2.csv")) {
                bool approximate(MultiPrecision<Pow2.N64> xmin, MultiPrecision<Pow2.N64> xmax) {
                    ...
                    for (MultiPrecision<Pow2.N64> u = umin, h = (umax - umin) / 8192; u <= umax; u += h) {
                        ...
                        if (x != 0) {
                            MultiPrecision<Pow2.N64> y = PDFN24.Value(1 / MultiPrecision<HoltsmarkExpected.N24>.Square(x.Convert<HoltsmarkExpected.N24>())).Convert<Pow2.N64>()
                                / MultiPrecision<Pow2.N64>.Pow(x, 5);

                            expecteds_range.Add((u, y));
                        }
                        else {
                            MultiPrecision<Pow2.N64> y = 3 / (4 * MultiPrecision<Pow2.N64>.Sqrt(2 * MultiPrecision<Pow2.N64>.PI));
